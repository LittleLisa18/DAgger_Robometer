...
def test_extract_prompt_from_task_field():
    transform = _transforms.PromptFromTaskField()

    data = transform({"task": np.asarray("Pack the items")})
    assert data["prompt"] == "Pack the items"
