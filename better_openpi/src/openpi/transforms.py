...
@dataclasses.dataclass(frozen=True)
class PromptFromTaskField(DataTransformFn):
    """Extracts a prompt from LeRobot's resolved task string field."""

    task_key: str = "task"

    def __call__(self, data: DataDict) -> DataDict:
        if self.task_key not in data:
            raise ValueError(f'Cannot extract prompt without "{self.task_key}"')

        prompt = data[self.task_key]
        if not isinstance(prompt, str):
