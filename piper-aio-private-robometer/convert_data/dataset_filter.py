...
def analyze_outliers_by_std(root_dir: str, data_key: str, std_threshold: float = 5.0):
    ...
    if not outlier_files:
        ...
        for file_name, info in sorted_files:
            print(
                f"{os.path.join(root_dir, file_name):<70} | {info['dim']:<10} | {info['value']:12.4f} | {info['z_score']:12.2f}"
            )
    print("-" * 120)
    return sorted_files
...
if __name__ == "__main__":
    ...
    outlier_files = analyze_outliers_by_std(root_dir, data_key="action", std_threshold=args.std_threshold)
