...
class Pi0Faster(_model.BaseModel):
    ...
    def sample_actions(
        ...
        *,
        ...
        alpha: float = 1.0,
        u0: float = 0.9,
    ) -> tuple[_model.Actions, at.Float[at.Array, "n b ah ad"], at.Float[at.Array, "m b ah ad"]]:
        ...
        def step(carry, _):
            ...
            x_next = x_t + dt * v_t
            return (x_next, time + dt), (x_next, v_t)
        ...
        def step_adaptive(carry, step_params):
            ...
            return (x_next, None), (x_next, v_t)

        if infer_time_schedule == "const":
            (x_0, _), (all_x_t, all_v_t) = jax.lax.scan(step, (noise, 1.0), None, length=num_steps)
            ...
            (x_0, _), (all_x_t, all_v_t) = jax.lax.scan(
                step_adaptive, (noise, None), (t_starts, dt_schedule), length=num_steps
            )
        else:
            raise ValueError(f"Invalid infer_time_schedule: {infer_time_schedule}")

        all_x_t = jnp.concatenate([noise[None, ...], all_x_t], axis=0)
        return x_0, all_x_t, all_v_t
